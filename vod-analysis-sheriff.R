...
# Create df
library(tibble)
test1_tbl <- as_tibble(test1)

library(tidyr)
test1_tbl <- test1_tbl %>% 
  # take Person_Stopped out of its nested structure and unfurl it
  # change data from one line per stop to one line per person in a stop 
...
# smash together, sort the numbers and return a character
jd18 = test1_tbl %>% 
...
# Time doesn't always have seconds if ending in ":00")
## Count number of characters in string to insert :00 when necessary
library(stringi)
...
# Remove original offenses (optional)
remove(offenses)
...
# Create tables of SD County center_lat and center_lng
...
# Middle Eastern / South Asian glm test
mesa_test <- glm(
...
# Calculate percent of race of individuals in all stops (July 2018 through June 2020)
race_all <- final %>% 
